...
import { Challenge, createChallenge } from "@/lib/challenge-store";
import { createChallengeInSupabase } from "@/lib/supabase-data";
...
export default function NewChallengePage() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const input: Omit<Challenge, "id" | "created_at"> = {
      ...form,
      miles_reward: Number(form.miles_reward),
    };

    try {
      const createdRemotely = await createChallengeInSupabase(input);
      if (createdRemotely) {
      ...
      <div className="mx-auto max-w-3xl">
        <header className="mb-8 flex items-center justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-black">Create challenge</h1>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 rounded-[28px] border border-white/10 bg-white/5 p-6">
          <div>
            ...
            <input
              ...
              placeholder="Example: 5K Easy Run"
              required
            />
          </div>
